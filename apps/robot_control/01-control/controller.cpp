...
#include <RotationLowPassFilter.h>
...
namespace {
	...
	// camera
	Vector3d CAMERA_POS_IN_LINK = Vector3d::Zero();
	Matrix3d CAMERA_ROT_IN_LINK = Matrix3d::Identity();
	Vector2d CAMERA_FOV = Vector2d::Zero();

	// proxy
	Vector3d CONTROL_POINT = Vector3d(0, 0, 0.107 + 0.014 + 0.017 + 0.028 + 0.035 + 0.097);
	Matrix3d CONTROL_ROTATION = Matrix3d::Identity();
...
std::string robot_file = std::string(URDF_PATH) + "/scenes/rizon4s.urdf";
...
std::atomic<bool> switch_zero_moment_control_flag(false);
...
bool isRotationMatrixValid(const Eigen::Matrix3d& R, const double EPSILON = 1e-6) {
    // Check if R is orthogonal: R * R^T = I
    Eigen::Matrix3d identity = Eigen::Matrix3d::Identity();
	if (!(R.transpose() * R).isApprox(identity, EPSILON)) {
        std::cout << "Matrix is not orthogonal.\n";
        return false;
    }

    // Check if determinant is +1
    double det = R.determinant();
    if (std::abs(det - 1.0) > EPSILON) {
        std::cout << "Determinant is not +1. Det: " << det << "\n";
        return false;
    }

    // If both conditions are met, it's a valid rotation matrix
    return true;
}

int main(int argc, char *argv[]) {

	// Sai2Model::URDF_FOLDERS["EXAMPLE_18_FOLDER"] = string(EXAMPLES_FOLDER) + "/18-panda_singularity";
	...
	if (config["camera"]) {
		YAML::Node current_node = config["camera"];
		std::vector<double> pos_in_link_vec = current_node["pos_in_link"].as<std::vector<double>>();
		CAMERA_POS_IN_LINK << pos_in_link_vec[0], pos_in_link_vec[1], pos_in_link_vec[2];

		std::vector<double> rot_in_link_vec = current_node["rot_in_link"].as<std::vector<double>>();
		CAMERA_ROT_IN_LINK << rot_in_link_vec[0], rot_in_link_vec[1], rot_in_link_vec[2], \
								rot_in_link_vec[3], rot_in_link_vec[4], rot_in_link_vec[5], \
								rot_in_link_vec[6], rot_in_link_vec[7], rot_in_link_vec[8];

		// std::vector<double> camera_fov_vec = current_node["fov"].as<std::vector<double>>>();
		// CAMERA_FOV << fov[0], fov[1];
	}
	...
	if (config["proxy"]) {
		...
		if (current_node) {
			...
			std::vector<double> control_rotation_vec = current_node["control_rotation"].as<std::vector<double>>();
			CONTROL_ROTATION = Matrix3d::Identity();
			CONTROL_ROTATION << control_rotation_vec[0], control_rotation_vec[1], control_rotation_vec[2], \
								control_rotation_vec[3], control_rotation_vec[4], control_rotation_vec[5], \
								control_rotation_vec[6], control_rotation_vec[7], control_rotation_vec[8];

			// verify that control_rotation is a valid rotation matrix
			if (!isRotationMatrixValid(CONTROL_ROTATION)) {
				throw runtime_error("Invalid control rotation matrix");
			}
	...
	*/
	...
	control_transform.linear() = CONTROL_ROTATION;
	...
	*/
	...
	Matrix3d global_proxy_ori = robot->rotation(control_link, CONTROL_ROTATION); 
	...
	*/
	...
	redis_client.setInt(RESET_VOXEL_KEY, 1);
	// debug
	redis_client.setEigen(CONTROL_FRAME_POS_KEY, motion_force_task->getCurrentPosition());
	redis_client.setEigen(CONTROL_FRAME_ORI_KEY, motion_force_task->getCurrentOrientation());
	...
	*/
	...
	while (runloop) {
		...
		force_moment.head(3) = redis_client.getEigen(FORCE_SENSOR_KEY);
		force_moment.tail(3) = redis_client.getEigen(MOMENT_SENSOR_KEY);
		...
		Matrix3d ee_ori = robot->rotation(control_link, CONTROL_ROTATION);
		...
		Vector3d camera_pos = robot->position(control_link, CAMERA_POS_IN_LINK);
		Matrix3d camera_ori = robot->rotation(control_link, CAMERA_ROT_IN_LINK);
		...
		redis_client.setEigen(CAMERA_POS_KEY, camera_pos);
		redis_client.setEigen(CAMERA_ORI_KEY, camera_ori);
		...
		*/
	    ...
	    if (ACTIVE_GRIPPER_CONTROL) {
			...
			if (redis_client.get(GRIPPER_MODE_KEY) == "g") {
			...
			} else if (redis_client.get(GRIPPER_MODE_KEY) == "o") {
				if (prev_grasped_object) {
					std::cout << time << ": RELEASED OBJECT\n";
					// set haptic clutch to prevent haptic force jerk when moving ?
					haptic_controller->setHapticControlType(Sai2Primitives::HapticControlType::CLUTCH);  // force the user to clutch again to prevent "jumps"
					// haptic_controller->setHapticControlType(Sai2Primitives::HapticControlType::MOTION_MOTION);
					// redis_client.set(HAPTIC_CLUTCH_KEY, "1");  // keep on recording through the released object phase 
			...
			// if (redis_client.get(GRIPPER_MODE_KEY) == "o" && prev_grasped_object == true) {
			// 	std::cout << time << ": RESETTING LOAD PARAMETERS\n";
			// 	load_mass = original_load_mass;
			// 	load_inertia = original_load_inertia;
			// 	load_com = original_load_com; 		
			...
			// 	// // instant reset to current robot position and orientation
			// 	// global_proxy_pos = ee_pos;
			// 	// global_proxy_ori = ee_ori;

			// 	// trigger frame interpolation 
			// 	flag_in_contact = true; 
			// 	loop_flag_force_to_free = true; 

			// 	// hold curernt robot position/orientation flag 
		...
		*/
		if (state == POSTURE) {
			...
			if (joint_task->goalPositionReached(QTOL) && haptic_controller->getHomed() && haptic_button_is_pressed) {
				...
				if (first_loop) {
					// if (!FLAG_SIMULATION && state == MOTION) {
						initial_force_moment_bias = 1 * force_moment;
			...
			*/
			...
			// v2: switch to using key press "m" for zero moment control switch 
			if (switch_zero_moment_control_flag.load()) {
				if (flag_in_contact) {
					if (!zero_moment_control_flag) {
						std::cout << time << ": ENABLING ZERO MOMENT CONTROL IN CONTACT\n";
						zero_moment_control_flag = true;
					} else if (zero_moment_control_flag) {
						std::cout << time << ": DISABLE ZERO MOMENT CONTROL IN CONTACT\n";
						zero_moment_control_flag = false;
					}
				}
				switch_zero_moment_control_flag.store(false);
			}
			...
			if (haptic_controller->getHapticControlType() == Sai2Primitives::HapticControlType::MOTION_MOTION &&
				...
				if (!flag_frame_reset && !flag_ori_frame_reset) {
					haptic_controller->setHapticControlType(Sai2Primitives::HapticControlType::CLUTCH);
				...
				// 	redis_client.set(HAPTIC_CLUTCH_KEY, "0");
				// } 
			} else if (switch_orientation_control_flag.load()) {
				if (haptic_controller->getOrientationTeleopEnabled()) {
				...
				switch_orientation_control_flag.store(false);
			} 
		...
		if (flag_in_contact) {
			// redis_client.setEigen(ENVIRONMENT_NORMAL_VECTOR_KEY, force_moment.head(3).normalized());
			redis_client.setEigen(ENVIRONMENT_NORMAL_VECTOR_KEY, motion_force_task->getGoalForce().normalized());
		} else {
			redis_client.setEigen(ENVIRONMENT_NORMAL_VECTOR_KEY, Vector3d::Zero());
		}
...
void particle_filter() {
	...
	std::vector<Vector3d> force_axes = {};
	int first_force_space_dimension = 0;
	...
	while (runloop) {
...

		{
			...
			first_force_space_dimension = force_dimension_queue.front();
			force_dimension_queue.pop();
...

		{
			...
			if (force_space_dimension == 0) {
				// transition to free space IF last popped has force dimension != 0, and all elements are 0 
				// if (all_elements_same && sigma_motion.norm() != 0) {
				if (all_elements_same && first_force_space_dimension != 0) {
...
void getKeyPressAndGraphics() {
	...
	graphics->showObjectLinkFrame(true, "control_frame", 0.15);
	...
	while (graphics->isWindowOpen()) {
		...
		if (key_pressed.at(GLFW_KEY_Q) && !key_was_pressed.at(GLFW_KEY_Q)) {
			switch_orientation_control_flag.store(true);
		}

		// zero moment control input switch
		if (key_pressed.at(GLFW_KEY_M) && !key_was_pressed.at(GLFW_KEY_M)) {
			switch_zero_moment_control_flag.store(true); 
		}
		...
		// debug for control frame
		Affine3d control_frame_transform;
		control_frame_transform.translation() = redis_client.getEigen(CONTROL_FRAME_POS_KEY);
		control_frame_transform.linear() = redis_client.getEigen(CONTROL_FRAME_ORI_KEY);
		graphics->updateObjectGraphics("control_frame", control_frame_transform);
