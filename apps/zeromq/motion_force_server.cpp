...
void compute_robot_joint_torques(std::shared_ptr<SaiModel::SaiModel> robot,
                                  ...
                                  float* torques_out, const float* des_cart_pos, const float* des_cart_orient, const float* force_or_motion_axis, const float* num_force_dim, const float* force_magnitude) {

    motion_force_task->setGoalForce(*force_magnitude);
    Vector3d f_or_m_axis = Vector3d(force_or_motion_axis[0], force_or_motion_axis[1], force_or_motion_axis[2]);
    motion_force_task->parametrizeForceMotionSpaces(static_cast<int>(*num_force_dim),f_or_m_axis);
...
int main() {
    ...
    while (true) {
        ...
        if (request.size() != num_envs * 26 * sizeof(float)) {
        ...
        for (int w = 0; w < num_workers; ++w) {
            workers.emplace_back([&, w]() {
                ...
                for (int i = start; i < end; ++i) {
                    ...
                    const float* force_or_motion_axis = joint_data + num_envs*21 + i *4;
                    const float* num_force_dim = force_or_motion_axis + 3;
                    const float* force_magnitude = num_force_dim + 1;
                
                    updateRobotState(robot_pool[i], qpos, qvel, mft_pool[i]);
                    compute_robot_joint_torques(robot_pool[i], mft_pool[i], jt_pool[i], output_torques + i * ROBOT_GRIPPER_JOINTS, pos, orient, force_or_motion_axis, force_magnitude);
                }
            });
        }
