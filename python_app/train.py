...
from policy_network import Agent, TransformerAgent
...
if __name__ == "__main__":
    ...
    agent = TransformerAgent(envs.obs_size, envs.obs_size, envs.obs_stack, action_space_dim).to(device)

    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)

    # ALGO Logic: Storage setup
    obs = torch.zeros((args.num_steps, args.num_envs, observation_space_dim)).to(device)
    actions = torch.zeros((args.num_steps, args.num_envs, action_space_dim)).to(device)
    ...
    _, next_obs  = envs.reset(mj_data)
    next_obs = torch.Tensor(next_obs).to(device)
    next_done = torch.zeros(args.num_envs).to(device)

    for iteration in range(0, args.num_iterations):
        print("Starting iteration {} of {}".format(iteration, args.num_iterations))
        ...
        for step in range(0, args.num_steps):
            ...
            mj_data, tup = envs.step(mj_data, action.cpu().numpy())
            next_obs, reward, next_done, infos = tup
        ...
        # flatten the batch
        b_obs = obs.reshape((-1,observation_space_dim))
        b_logprobs = logprobs.reshape(-1)
        b_actions = actions.reshape((-1,action_space_dim))
